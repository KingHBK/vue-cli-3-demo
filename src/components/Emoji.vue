...
export default {
  data () {
    return {
      ...
      emojiBaseUrl: 'http://yx-web.nosdn.127.net/webdoc/h5/emoji/emoji'
    }
  },
  methods: {
    clickEmoji (name) {
    ...
    handleEmojiText (sourceText) {
      ...
      if (/\[[^\]]+\]/.test(sourceText)) {
        ...
        emojiItems.forEach(text => {
          let emojiCnt = this.emojiList
          if (emojiCnt[text]) {
            sourceText = sourceText.replace(
              text,
              `<img style="width: 16px; height: 16px;" src="${this.emojiBaseUrl}/${emojiCnt[text].file}">`
            )
          }
        })
      }
